...
//        $decodingJwt->decoding($user->getApiToken());
     ...
     * @throws ApiUnavailableException
     */
    public function adminka($user): UserDTO
    {
        // Запрос в сервис биллинг
        $query = curl_init('http://' . $this->startUri . ':82/login');
        curl_setopt($query, CURLOPT_POST, 1);
        curl_setopt($query, CURLOPT_USERPWD, $user->getEmail() . ":" . $user->getPassword());
        curl_setopt($query, CURLOPT_POSTFIELDS, $user);
        curl_setopt($query, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($query, CURLOPT_HTTPHEADER, [
